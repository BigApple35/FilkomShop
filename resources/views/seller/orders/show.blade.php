...
<head>
    ...
    <title>Detail Pesanan - FILKOMSHOP</title>
...
<body>
    ...
    <div class="fs-container">
        <div class="fs-header">
            <a href="{{ route('seller.orders.index') }}" class="fs-btn-back">Back</a>
            <h1 class="fs-title">Order Details</h1>
        </div>
        <div class="fs-card">
            <div class="fs-info-group">
                <div class="fs-info-label">Order ID</div>
                <div class="fs-info-value" style="font-weight: 700;">#ORD-{{ $order->id }}</div>
            </div>
            <div class="fs-info-group">
                <div class="fs-info-label">Order Status</div>
                <span class="fs-badge fs-badge-{{ strtolower($order->status) }}">{{ $order->status }}</span>
            </div>
            <div class="fs-info-group">
                <div class="fs-info-label">Transaction Date</div>
                <div class="fs-info-value">{{ \Carbon\Carbon::parse($order->ordered_at ?? $order->created_at)->format('d F Y, H:i') }} WIB</div>
            </div>
            <div class="fs-info-group">
                <div class="fs-info-label">Shipping Address</div>
                <div class="fs-info-value">{{ $order->shipping_address }}</div>
            </div>
            <hr class="fs-divider">
            <div class="fs-info-group" style="margin-bottom: 0;">
                <div class="fs-info-label">Total Amount</div>
                <div class="fs-total-price">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</div>
            </div>
        </div>
    </div>
</body>
