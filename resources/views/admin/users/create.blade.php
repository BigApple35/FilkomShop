...
<head>
    ...
    <title>Tambah Pengguna - FILKOMSHOP Admin</title>
    ...
    <div class="max-w-7xl mx-auto px-6 pt-28 pb-10">
        <div class="bg-white rounded-[2rem] border border-slate-200 shadow-xl p-8 max-w-3xl mx-auto">
            ...
            <div class="flex flex-col gap-6 sm:flex-row sm:items-center sm:justify-between mb-8">
                <div>
                    <h1 class="text-5xl font-bold text-[#1f232b]">Create New User</h1>
                    <p class="text-slate-500 mt-3">Tambahkan pengguna baru ke dalam sistem katalog FILKOMSHOP.</p>
                </div>
            ...
            <form action="{{ route('admin.users.store') }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <input id="name" name="name" type="text" value="{{ old('name') }}" class="w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-slate-900 focus:outline-none focus:border-slate-400" required />
                    @error('name') <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2" for="email">Email Address</label>
                    <input id="email" name="email" type="email" value="{{ old('email') }}" class="w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-slate-900 focus:outline-none focus:border-slate-400" required />
                    @error('email') <span class="text-red-500 text-sm mt-1 block">{{ $message }}</span> @enderror
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2" for="password">Password</label>
                    <input id="password" name="password" type="password" class="w-full rounded-3xl border border-slate-200 bg-white px-4 py-3 text-slate-900 focus:outline-none focus:border-slate-400" required />
